...
install() {
  ...
  if(mkdir(INSTALL_DIR"/sce_module", 0755) && errno != EEXIST) {
    orbis_notify("Unable to create sce_module dir");
    return;
  }

  snprintf(path, sizeof path, "%s/app0/sce_module/libSceFios2.prx", cwd);
  if(copy_file(path, INSTALL_DIR"/sce_module/libSceFios2.prx")) {
    orbis_notify("Unable to install %s", path);
    return;
  }

  snprintf(path, sizeof path, "%s/app0/sce_module/libc.prx", cwd);
  if(copy_file(path, INSTALL_DIR"/sce_module/libc.prx")) {
    orbis_notify("Unable to install %s", path);
    return;
  }
...
int main() {
  ...
  /* Give the notification time to become visible, then leave the installer
   * cleanly. The next tile launch executes the installed Dropbear daemon. */
  sleep(2);
  if(sceSystemServiceLoadExec("exit", NULL) < 0) {
    orbis_notify("Installation beendet - bitte Anwendung schliessen");
